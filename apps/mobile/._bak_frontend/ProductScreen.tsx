...
export default function ProductScreen({ route, navigation }: Props) {
  ...
  const [tab, setTab] = useState<TabKey>((route.params?.initialTab as TabKey) ?? "Health");
  useEffect(() => {
    const desired = (route.params?.initialTab as TabKey) ?? "Health";
    setTab(desired);
  }, [route.params?.barcode, route.params?.initialTab]);
